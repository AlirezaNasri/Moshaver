...
def get_all_workshops(req):
    ...
    context = get_context(page + 1 if len(Workshop.objects.all()) > page * 10 else None,
...
def get_all_edus(req):
    ...
    if 'page' in req.GET:
        page = int(req.GET['page'])
    context = get_context(page + 1 if len(EduContent.objects.all()) > page * 10 else None,
                          page - 1 if page != 1 else None)
...
def get_all_grads(req):
    context = get_context()
    fields = {}
    for grad in Grad.objects.all().order_by('row'):
...
def get_gallery(req):
    ...
    if 'page' in req.GET:
        page = int(req.GET['page'])
    context = get_context(
        page + 1 if len(ImageContent.objects.filter(caption__startswith='gallery')) > page * 10 else None,
        page - 1 if page != 1 else None)
